...
addEventListener("click", function(event)
{
    ...
    if (link && link.hash) {
        ...
        if(strstr(link.hash.substr(1), 'courses')) {
          window.lastpage = link.hash.substr(1);
          
          $('#homeButton').html("Courses");
          $('#homeButton').attr("href", "#home");
        }
        
        showPage(page);
        
        if(strstr(link.hash.substr(1), 'reading')) {
          $('#homeButton').html("Back");
          $('#homeButton').attr("href", "#"+window.lastpage);
        }
    }
}, true);
