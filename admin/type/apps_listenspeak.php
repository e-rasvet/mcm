...
if ($act == "delete" && !empty($id))
    $db->query("DELETE FROM {$CFG['prefix']}apps_listenspeak WHERE id = ".$id);


if ($category) {
    $soundtexttype = 0;
    
    if ($_FILES['soundtext']['tmp_name'] || is_file($CFG['dirroot'].'/datas/listenspeak/soundtext/'.$id.'.mp3')) $soundtexttype = 1;
    ...
    $id = insert_record("apps_listenspeak", $datam);
    
    if ($_FILES['soundtext']['tmp_name']) move_uploaded_file ($_FILES['soundtext']['tmp_name'], $CFG['dirroot']."/datas/listenspeak/soundtext/{$id}.mp3");
    
    $status = statusmessage($word.' - Added');
}

if ($act == "edit" && !empty($id))
    $data = get_record("apps_listenspeak", array("id"=>$id));
?>
...
function validate_form(form) { 
	...
	<!--  start page-heading -->
	<div id="page-heading">
		<h1><?php if ($data->id) echo "Edit"; else echo "Add"; ?> listenspeak item</h1>
	</div>
	<!-- end page-heading -->
	...
	<td>
		...
		<div id="step-holder">
			...
			<div class="step-dark-left"><a href="#">Add listenspeak item details</a></div>
			<div class="step-dark-right">&nbsp;</div>
			<div class="step-no-off"></div>
			<div class="step-light-left"><a href="http://webapp.netcourse.org/products.php#listenspeak" target="_blank">Fields help</a></div>
		...
		<!-- start id-form -->
		<form id="formadd" name="formadd_point" enctype="multipart/form-data" method="post" action="?type=apps_listenspeak" onsubmit="return validate_form(this)">
	...
	var options = {
		script:"<?php echo $CFG['wwwroot']; ?>/admin/type/category_ajax.php?t=apps_listenspeak&",
		varname:"input",
		json:true
	};
	var as_json = new AutoSuggest('catsuggest', options);
		...
		<tr>
			...
			<td>
			<?php  if (!empty($data->soundtext)) echo '<div style="margin-left:20px;"><a href="'.$CFG['wwwroot'].'/datas/listenspeak/soundtext/'.$id.'.mp3" target="_blank">audio</a></div>'; else {?>
